...
boru-driven command-line client **and** interactive REPL for the IpinfoDeveloper
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ipinfo-developer-cli)
make build

# 2. See usage (words, entities, env vars)
./ipinfo-developer-cli --help

# 3. Provide credentials once, via the environment
export IPINFO_DEVELOPER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ipinfo-developer-cli load 1 abuse            # {id:1} shorthand
./ipinfo-developer-cli load '{id:1}' abuse       # explicit match map

# 5. Override the API base URL for a single call
IPINFO_DEVELOPER_BASE=https://api.example.com ./ipinfo-developer-cli load 1 abuse

# 6. No arguments -> interactive REPL
./ipinfo-developer-cli
ipinfo-developer> load 1 abuse
ipinfo-developer> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ipinfo-developer-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ipinfo-developer-cli load 1 abuse
   ```
...
```sh
./ipinfo-developer-cli load 1 abuse          # scalar shorthand for {id:1}
./ipinfo-developer-cli load '{id:1}' abuse     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ipinfo-developer>`). Each line is
evaluated as its own boru expression:

```text
$ ./ipinfo-developer-cli
ipinfo-developer> load 1 abuse
ipinfo-developer> /help
ipinfo-developer> /quit
```
...
below — this SDK exposes 28 entities.
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `abuse`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ipinfo-developer-cli`. |
...
The 28 entities this SDK exposes (any is valid as `<entity>`):
...
(`./ipinfo-developer-cli <expr>`) and interactively (the REPL), and expressions compose the
