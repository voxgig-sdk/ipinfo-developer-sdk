// Command IpinfoDeveloper-cli is a CLI + REPL for the IpinfoDeveloper SDK,
...
const prompt = "ipinfo-developer"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "abuse asn carrier company core domain general get_current_information get_information_by_ip ipinfo_core ipinfo_lite ipinfo_plus lite max men place plus privacy privacy_extended range residential_proxy single whois_asn whois_domain whois_ip whois_net_id whois_org whois_poc"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IPINFO_DEVELOPER_APIKEY (api key), IPINFO_DEVELOPER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IPINFO_DEVELOPER_APIKEY carries the API key and
	// IPINFO_DEVELOPER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IPINFO_DEVELOPER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IPINFO_DEVELOPER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpinfoDeveloperSDK(opts)
