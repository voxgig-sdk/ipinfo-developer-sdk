...
class PrivacyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = privacy_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "privacy." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_PRIVACY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    privacy_ref01_data = nil
    if privacy_ref01_data_raw.length > 0
...
def privacy_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "privacy", "PrivacyTestData.json")
  ...
  entid_env_raw = ENV["IPINFO_DEVELOPER_TEST_PRIVACY_ENTID"]
