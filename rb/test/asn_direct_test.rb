...
class AsnDirectTest < Minitest::Test
  def test_direct_list_asn
    setup = asn_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-asn", setup[:live] ? "live" : "unit")
    if _should_skip
