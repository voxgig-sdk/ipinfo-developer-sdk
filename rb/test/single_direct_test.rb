...
class SingleDirectTest < Minitest::Test
  def test_direct_load_single
    setup = single_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-single", setup[:live] ? "live" : "unit")
    if _should_skip
...
def single_direct_setup(mockres)
