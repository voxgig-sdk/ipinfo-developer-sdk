...
class WhoisAsnDirectTest < Minitest::Test
  def test_direct_list_whois_asn
    setup = whois_asn_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-whois_asn", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["asn01"].each do |_live_key|
...
def whois_asn_direct_setup(mockres)
