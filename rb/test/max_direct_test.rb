...
class MaxDirectTest < Minitest::Test
  def test_direct_load_max
    setup = max_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-max", setup[:live] ? "live" : "unit")
    if _should_skip
