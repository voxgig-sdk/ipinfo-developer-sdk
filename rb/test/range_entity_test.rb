...
class RangeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = range_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "range." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_RANGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    range_ref01_data = nil
    if range_ref01_data_raw.length > 0
...
def range_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "range", "RangeTestData.json")
  ...
  entid_env_raw = ENV["IPINFO_DEVELOPER_TEST_RANGE_ENTID"]
