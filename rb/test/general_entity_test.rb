...
class GeneralEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_GENERAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    general_ref01_data_result, err = general_ref01_ent.create(general_ref01_data, nil)
    assert_nil err
...
def general_basic_setup(extra)
  ...
  entid_env_raw = ENV["IPINFODEVELOPER_TEST_GENERAL_ENTID"]
