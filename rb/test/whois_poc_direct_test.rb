...
class WhoisPocDirectTest < Minitest::Test
  def test_direct_load_whois_poc
    setup = whois_poc_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-whois_poc", setup[:live] ? "live" : "unit")
    if _should_skip
...
def whois_poc_direct_setup(mockres)
