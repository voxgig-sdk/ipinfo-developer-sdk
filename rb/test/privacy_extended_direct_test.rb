...
class PrivacyExtendedDirectTest < Minitest::Test
  def test_direct_list_privacy_extended
    setup = privacy_extended_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-privacy_extended", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["ip01"].each do |_live_key|
...
def privacy_extended_direct_setup(mockres)
