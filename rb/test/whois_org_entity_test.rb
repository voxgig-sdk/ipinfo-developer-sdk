...
class WhoisOrgEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = whois_org_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "whois_org." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_WHOIS_ORG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    whois_org_ref01_data = nil
    if whois_org_ref01_data_raw.length > 0
...
def whois_org_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "whois_org", "WhoisOrgTestData.json")
  ...
  entid_env_raw = ENV["IPINFO_DEVELOPER_TEST_WHOIS_ORG_ENTID"]
