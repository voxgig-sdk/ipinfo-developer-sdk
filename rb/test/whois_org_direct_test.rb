...
class WhoisOrgDirectTest < Minitest::Test
  def test_direct_load_whois_org
    setup = whois_org_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-whois_org", setup[:live] ? "live" : "unit")
    if _should_skip
...
def whois_org_direct_setup(mockres)
