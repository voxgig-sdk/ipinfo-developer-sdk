...
class WhoisNetIdEntity
  ...
  def make
    opts = @_entopts.dup
    WhoisNetIdEntity.new(@_client, opts)
  end
  ...
  # @return [WhoisNetId, Hash] the current WhoisNetId data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of WhoisNetId fields)
  def match_get
