...
class WhoisIpEntity
  ...
  def make
    opts = @_entopts.dup
    WhoisIpEntity.new(@_client, opts)
  end
  ...
  # @return [WhoisIp, Hash] the current WhoisIp data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of WhoisIp fields)
  def match_get
  ...
  #
  # @param reqmatch [WhoisIpLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.WhoisIp.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [WhoisIp, Hash] the loaded WhoisIp; raises IpinfoDeveloperError on failure
