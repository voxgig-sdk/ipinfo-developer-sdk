...
class GetCurrentInformationEntity
  ...
  def make
    opts = @_entopts.dup
    GetCurrentInformationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpinfoDeveloperHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetCurrentInformation, Hash] the current GetCurrentInformation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpinfoDeveloperHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetCurrentInformation fields)
  def match_get
  ...
  #
  # @param reqmatch [GetCurrentInformationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetCurrentInformation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetCurrentInformation, Hash] the loaded GetCurrentInformation; raises IpinfoDeveloperError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpinfoDeveloperHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
