...
module IpinfoDeveloperUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpinfoDeveloperResponse.new({})
    ...
    if fetch_err
      ...
      response = IpinfoDeveloperResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpinfoDeveloperResponse.new(fetched)
    else
