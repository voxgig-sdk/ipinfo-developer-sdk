...
//    only - a typed IpinfoDeveloperContext there would make every ctx
...
final class IpinfoDeveloperOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpinfoDeveloperContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IpinfoDeveloperSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IpinfoDeveloperResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IpinfoDeveloperError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IpinfoDeveloperResponse($clean['response']);
    ...
    public static function err_from_map($m): ?IpinfoDeveloperError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpinfoDeveloperError(is_string($code) ? $code : '', $msg);
    }
