...
class WhoisOrgEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpinfoDeveloperSDK::test(null, null);
        $ent = $testsdk->WhoisOrg(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = whois_org_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "whois_org." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_WHOIS_ORG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $whois_org_ref01_data = null;
        if (count($whois_org_ref01_data_raw) > 0) {
        ...
        [$whois_org_ref01_data_dt0_loaded, $err] = $whois_org_ref01_ent->load($whois_org_ref01_match_dt0, null);
        $this->assertNull($err);
...
function whois_org_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/whois_org/WhoisOrgTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["whois_org01", "whois_org02", "whois_org03"] as $k) {
    ...
    $entid_env_raw = getenv("IPINFODEVELOPER_TEST_WHOIS_ORG_ENTID");
