...
class RangeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpinfoDeveloperSDK::test(null, null);
        $ent = $testsdk->Range(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = range_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "range." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_RANGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $range_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.range")));
        $range_ref01_data = null;
        if (count($range_ref01_data_raw) > 0) {
            $range_ref01_data = Helpers::to_map($range_ref01_data_raw[0][1]);
        }
...
function range_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/range/RangeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["range01", "range02", "range03"] as $k) {
    ...
    $entid_env_raw = getenv("IPINFO_DEVELOPER_TEST_RANGE_ENTID");
