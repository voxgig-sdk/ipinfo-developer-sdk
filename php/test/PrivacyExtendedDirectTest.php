...
class PrivacyExtendedDirectTest extends TestCase
{
    public function test_direct_list_privacy_extended(): void
    {
        $setup = privacy_extended_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-privacy_extended", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["ip01"] as $_liveKey) {
