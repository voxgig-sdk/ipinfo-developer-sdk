...
class WhoisIpEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_WHOIS_IP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $whois_ip_ref01_data_dt0_loaded = $whois_ip_ref01_ent->load($whois_ip_ref01_match_dt0, null);
...
function whois_ip_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("IPINFO_DEVELOPER_TEST_WHOIS_IP_ENTID");
