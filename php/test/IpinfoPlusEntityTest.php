...
class IpinfoPlusEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpinfoDeveloperSDK::test(null, null);
        $ent = $testsdk->IpinfoPlus(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = ipinfo_plus_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ipinfo_plus." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_IPINFO_PLUS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ipinfo_plus_ref01_data = null;
        if (count($ipinfo_plus_ref01_data_raw) > 0) {
        ...
        [$ipinfo_plus_ref01_data_dt0_loaded, $err] = $ipinfo_plus_ref01_ent->load($ipinfo_plus_ref01_match_dt0, null);
        $this->assertNull($err);
...
function ipinfo_plus_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ipinfo_plus/IpinfoPlusTestData.json';
    ...
    $entid_env_raw = getenv("IPINFODEVELOPER_TEST_IPINFO_PLUS_ENTID");
