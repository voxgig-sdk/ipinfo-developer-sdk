...
class MaxEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpinfoDeveloperSDK::test(null, null);
        $ent = $testsdk->Max(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = max_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "max." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_MAX_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function max_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/max/MaxTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["max01", "max02", "max03"] as $k) {
    ...
    $entid_env_raw = getenv("IPINFO_DEVELOPER_TEST_MAX_ENTID");
