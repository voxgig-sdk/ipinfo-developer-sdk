...
// IpinfoDeveloper SDK feature test
//
...
class FtCtrl extends IpinfoDeveloperControl
{
...
class FtClient
{
    ...
    private IpinfoDeveloperUtility $utility;

    public function __construct(IpinfoDeveloperUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpinfoDeveloperUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpinfoDeveloperContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpinfoDeveloperContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpinfoDeveloperContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpinfoDeveloperConfig::make_feature($name);
    ...
    private static function buildUrl(IpinfoDeveloperSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpinfoDeveloperError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpinfoDeveloperResponse($fetched);
        }
    ...
    private function fail(IpinfoDeveloperContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpinfoDeveloperContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
