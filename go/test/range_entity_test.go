...
func TestRangeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		rangeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.range")))
...
func rangeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFO_DEVELOPER_TEST_RANGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFO_DEVELOPER_TEST_RANGE_ENTID"])
