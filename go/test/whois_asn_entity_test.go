...
func TestWhoisAsnEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.WhoisAsn(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.WhoisAsn(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_WHOIS_ASN_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		whoisAsnRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.whois_asn")))
...
func whois_asnBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFO_DEVELOPER_TEST_WHOIS_ASN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFO_DEVELOPER_TEST_WHOIS_ASN_ENTID"])
