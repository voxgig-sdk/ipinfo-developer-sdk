...
func TestWhoisNetIdEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		whoisNetIdRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.whois_net_id")))
...
func whois_net_idBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFO_DEVELOPER_TEST_WHOIS_NET_ID_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFO_DEVELOPER_TEST_WHOIS_NET_ID_ENTID"])
