...
func TestIpinfoCoreEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.IpinfoCore(nil)
		if ent == nil {
			t.Fatal("expected non-nil IpinfoCoreEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := ipinfo_coreBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "ipinfo_core." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_IPINFO_CORE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ipinfo_coreBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "ipinfo_core", "IpinfoCoreTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read ipinfo_core test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse ipinfo_core test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("IPINFODEVELOPER_TEST_IPINFO_CORE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFODEVELOPER_TEST_IPINFO_CORE_ENTID"])
