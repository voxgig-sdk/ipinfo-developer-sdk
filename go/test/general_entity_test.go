...
func TestGeneralEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_GENERAL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		generalRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "general"}, setup.data), "general_ref01"))

		generalRef01DataResult, err := generalRef01Ent.Create(generalRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		generalRef01Data = core.ToMapAny(entityData(generalRef01DataResult))
		if generalRef01Data == nil {
...
func generalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFO_DEVELOPER_TEST_GENERAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFO_DEVELOPER_TEST_GENERAL_ENTID"])
