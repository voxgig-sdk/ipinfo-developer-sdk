...
func TestGeneralEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_GENERAL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		generalRef01Data = core.ToMapAny(generalRef01DataResult)
...
func generalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFODEVELOPER_TEST_GENERAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFODEVELOPER_TEST_GENERAL_ENTID"])
