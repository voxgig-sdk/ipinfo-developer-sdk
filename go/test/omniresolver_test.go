...
import (
	...
	omni "github.com/voxgig-sdk/ipinfo-developer-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpinfoDeveloperError); is {
				out := map[string]any{
					"name":    "IpinfoDeveloperError",
...
func omniCtx(arg any, client *sdk.IpinfoDeveloperSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpinfoDeveloperSDK); is {
			client = live
