...
func TestWhoisPocEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		whoisPocRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.whois_poc")))
...
func whois_pocBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINFO_DEVELOPER_TEST_WHOIS_POC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINFO_DEVELOPER_TEST_WHOIS_POC_ENTID"])
