...
class TestGetInformationByIpDirect:

    def test_should_direct_load_get_information_by_ip(self):
        setup = _get_information_by_ip_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_information_by_ip", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_information_by_ip_direct_setup(mockres):
