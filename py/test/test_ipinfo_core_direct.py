...
class TestIpinfoCoreDirect:

    def test_should_direct_load_ipinfo_core(self):
        setup = _ipinfo_core_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ipinfo_core", "live" if setup["live"] else "unit")
        if _skip:
...
def _ipinfo_core_direct_setup(mockres):
