...
class TestResidentialProxyDirect:

    def test_should_direct_load_residential_proxy(self):
        setup = _residential_proxy_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-residential_proxy", "live" if setup["live"] else "unit")
        if _skip:
...
def _residential_proxy_direct_setup(mockres):
