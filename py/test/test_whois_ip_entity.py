    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IPINFODEVELOPER_TEST_WHOIS_IP_ENTID JSON to run live")
        client = setup["client"]
        ...
        whois_ip_ref01_data_dt0_loaded, err = whois_ip_ref01_ent.load(whois_ip_ref01_match_dt0, None)
        assert err is None
...
def _whois_ip_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IPINFODEVELOPER_TEST_WHOIS_IP_ENTID"))
