...
class TestMaxDirect:
    ...
    def test_should_direct_load_max(self):
        setup = _max_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-max", "live" if setup["live"] else "unit")
        if _skip:
