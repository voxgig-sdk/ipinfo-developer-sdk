...
class TestMaxEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Max(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _max_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "max." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IPINFO_DEVELOPER_TEST_MAX_ENTID JSON to run live")
        client = setup["client"]
        ...
        max_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(max_ref01_data_dt0_loaded))
        assert max_ref01_data_dt0_load_result is not None
        assert max_ref01_data_dt0_load_result["id"] == max_ref01_data["id"]



def _max_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/max/MaxTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "IPINFO_DEVELOPER_TEST_MAX_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("IPINFO_DEVELOPER_TEST_MAX_ENTID"))
