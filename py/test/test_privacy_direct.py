...
class TestPrivacyDirect:

    def test_should_direct_load_privacy(self):
        setup = _privacy_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-privacy", "live" if setup["live"] else "unit")
        if _skip:
...
def _privacy_direct_setup(mockres):
