...
class TestWhoisPocDirect:

    def test_should_direct_load_whois_poc(self):
        setup = _whois_poc_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-whois_poc", "live" if setup["live"] else "unit")
        if _skip:
...
def _whois_poc_direct_setup(mockres):
