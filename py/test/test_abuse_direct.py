...
class TestAbuseDirect:

    def test_should_direct_load_abuse(self):
        setup = _abuse_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-abuse", "live" if setup["live"] else "unit")
        if _skip:
...
def _abuse_direct_setup(mockres):
