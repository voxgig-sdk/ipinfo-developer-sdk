...
class TestPlusDirect:

    def test_should_direct_load_plus(self):
        setup = _plus_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-plus", "live" if setup["live"] else "unit")
        if _skip:
...
def _plus_direct_setup(mockres):
