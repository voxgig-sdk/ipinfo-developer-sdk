...
class TestWhoisIpDirect:

    def test_should_direct_load_whois_ip(self):
        setup = _whois_ip_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-whois_ip", "live" if setup["live"] else "unit")
        if _skip:
...
def _whois_ip_direct_setup(mockres):
