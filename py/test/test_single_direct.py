...
class TestSingleDirect:
    ...
    def test_should_direct_load_single(self):
        setup = _single_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-single", "live" if setup["live"] else "unit")
        if _skip:
...
def _single_direct_setup(mockres):
