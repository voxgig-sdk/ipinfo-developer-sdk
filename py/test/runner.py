# IpinfoDeveloper SDK test runner
...
class IpinfoDeveloperTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpinfoDeveloperTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpinfoDeveloperTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpinfoDeveloperTestRunner.getenv("IPINFO_DEVELOPER_TEST_LIVE")
        override = IpinfoDeveloperTestRunner.getenv("IPINFO_DEVELOPER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpinfoDeveloperTestRunner.getenv(key)
        ...
        explain = IpinfoDeveloperTestRunner.getenv("IPINFO_DEVELOPER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IpinfoDeveloperTestRunner._test_control is not None:
            return IpinfoDeveloperTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpinfoDeveloperTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpinfoDeveloperTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpinfoDeveloperTestRunner.load_test_control()
...
def load_env_local():
    IpinfoDeveloperTestRunner.load_env_local()


def env_override(m):
    return IpinfoDeveloperTestRunner.env_override(m)


def entity_data(v):
    return IpinfoDeveloperTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpinfoDeveloperTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpinfoDeveloperTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpinfoDeveloperTestRunner.load_test_control()


def live_client_options():
    return IpinfoDeveloperTestRunner.live_client_options()


def live_delay_ms():
    return IpinfoDeveloperTestRunner.live_delay_ms()
