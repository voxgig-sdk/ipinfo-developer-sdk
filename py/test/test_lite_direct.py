...
class TestLiteDirect:

    def test_should_direct_load_lite(self):
        setup = _lite_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-lite", "live" if setup["live"] else "unit")
        if _skip:
...
def _lite_direct_setup(mockres):
