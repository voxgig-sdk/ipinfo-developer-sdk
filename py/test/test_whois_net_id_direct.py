...
class TestWhoisNetIdDirect:

    def test_should_direct_load_whois_net_id(self):
        setup = _whois_net_id_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-whois_net_id", "live" if setup["live"] else "unit")
        if _skip:
...
def _whois_net_id_direct_setup(mockres):
