...
class TestIpinfoLiteDirect:

    def test_should_direct_load_ipinfo_lite(self):
        setup = _ipinfo_lite_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ipinfo_lite", "live" if setup["live"] else "unit")
        if _skip:
...
def _ipinfo_lite_direct_setup(mockres):
