...
class TestWhoisDomainDirect:

    def test_should_direct_load_whois_domain(self):
        setup = _whois_domain_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-whois_domain", "live" if setup["live"] else "unit")
        if _skip:
...
def _whois_domain_direct_setup(mockres):
