...
class TestRangeDirect:

    def test_should_direct_load_range(self):
        setup = _range_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-range", "live" if setup["live"] else "unit")
        if _skip:
...
def _range_direct_setup(mockres):
