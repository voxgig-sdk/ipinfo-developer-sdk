...
class ResidentialProxyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ResidentialProxyEntity(self._client, opts)
    ...
    def data_get(self) -> ResidentialProxy:
    ...
    def match_get(self) -> ResidentialProxy:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ResidentialProxy:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ResidentialProxy().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
