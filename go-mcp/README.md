...
[MCP](https://modelcontextprotocol.io) server exposing the IpinfoDeveloper SDK as
two agent tools — `ipinfo-developer_list` and `ipinfo-developer_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ipinfo-developer-mcp)
make build

# 2. Provide credentials via the environment
export IPINFO_DEVELOPER_APIKEY=sk_live_xxx
...
```jsonc
// ipinfo-developer_list: first page of records
{ "entity": "asn" }
{ "entity": "asn", "query": { } }

// ipinfo-developer_load: one record by id
{ "entity": "abuse", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ipinfo-developer-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ipinfo-developer \
     -- "$PWD"/dist/*/ipinfo-developer-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ipinfo-developer_list` and `ipinfo-developer_load` tools now appear
   in new sessions. Ask the agent to *"list asn using ipinfo-developer"*
   and it calls `ipinfo-developer_list` with `{"entity":"asn"}`.
...
```sh
export IPINFO_DEVELOPER_APIKEY=sk_live_xxx            # API key
export IPINFO_DEVELOPER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ipinfo-developer_list` tool
...
### Call the `ipinfo-developer_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ipinfo-developer_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ipinfo-developer_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 28 supported entities (see below). |
...
| Variable | Purpose |
|----------|---------|
| `IPINFO_DEVELOPER_APIKEY` | API key sent with every request. |
| `IPINFO_DEVELOPER_BASE` | Optional override of the API base URL. |

### Entities

The 28 entities valid as the `entity` argument:
