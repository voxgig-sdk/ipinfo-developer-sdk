...
describe("AsnDirect", function()
  it("should direct-list-asn", function()
    local setup = asn_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-asn", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"asn01"}) do
...
function asn_direct_setup(mockres)
