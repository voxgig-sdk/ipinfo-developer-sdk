...
describe("CoreEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set IPINFODEVELOPER_TEST_CORE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(core_ref01_data_dt0_loaded)
...
function core_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("IPINFODEVELOPER_TEST_CORE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["IPINFODEVELOPER_TEST_CORE_ENTID"])
