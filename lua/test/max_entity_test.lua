...
describe("MaxEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Max(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = max_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "max." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_MAX_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local max_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.max")))
    local max_ref01_data = nil
    if #max_ref01_data_raw > 0 then
      max_ref01_data = helpers.to_map(max_ref01_data_raw[1][2])
    end
...
function max_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/max/MaxTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read max test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("IPINFO_DEVELOPER_TEST_MAX_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["IPINFO_DEVELOPER_TEST_MAX_ENTID"])
