...
local LiteEntity = {}
LiteEntity.__index = LiteEntity


function LiteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LiteEntity)
  self._name = "lite"
  self._client = client
...
function LiteEntity:get_name()
  return self._name
end


function LiteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LiteEntity.new(self._client, opts)
end
...
function LiteEntity:mark_deleted()
  self._deleted = true
end


function LiteEntity:deleted()
  return true == self._deleted
end


function LiteEntity:data_set(args)
  if args ~= nil then
...
function LiteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LiteEntity:match_set(args)
  if args ~= nil then
...
function LiteEntity:stream(action, args, callopts)
...
---@param reqmatch LiteLoadMatch
---@param ctrl? table
---@return Lite
---@return string? err
function LiteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LiteEntity:_run_op(ctx, post_done)
  local utility = self._utility
