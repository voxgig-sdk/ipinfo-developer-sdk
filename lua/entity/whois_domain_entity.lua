...
local WhoisDomainEntity = {}
WhoisDomainEntity.__index = WhoisDomainEntity


function WhoisDomainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisDomainEntity)
  self._name = "whois_domain"
  self._client = client
...
function WhoisDomainEntity:get_name()
  return self._name
end


function WhoisDomainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisDomainEntity.new(self._client, opts)
end
...
function WhoisDomainEntity:mark_deleted()
  self._deleted = true
end


function WhoisDomainEntity:deleted()
  return true == self._deleted
end


function WhoisDomainEntity:data_set(args)
  if args ~= nil then
...
function WhoisDomainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisDomainEntity:match_set(args)
  if args ~= nil then
...
function WhoisDomainEntity:match_get()
...
function WhoisDomainEntity:stream(action, args, callopts)
...
---@param reqmatch WhoisDomainLoadMatch
---@param ctrl? table
---@return WhoisDomain
---@return string? err
function WhoisDomainEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WhoisDomainEntity:_run_op(ctx, post_done)
  local utility = self._utility
