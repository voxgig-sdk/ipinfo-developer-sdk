...
local SingleEntity = {}
SingleEntity.__index = SingleEntity


function SingleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SingleEntity)
  self._name = "single"
  self._client = client
...
function SingleEntity:get_name()
  return self._name
end


function SingleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SingleEntity.new(self._client, opts)
end
...
function SingleEntity:mark_deleted()
  self._deleted = true
end


function SingleEntity:deleted()
  return true == self._deleted
end


function SingleEntity:data_set(args)
  if args ~= nil then
...
function SingleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SingleEntity:match_set(args)
  if args ~= nil then
...
function SingleEntity:match_get()
...
function SingleEntity:stream(action, args, callopts)
...
function SingleEntity:_run_op(ctx, post_done)
  local utility = self._utility
