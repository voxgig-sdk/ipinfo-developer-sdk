...
local RangeEntity = {}
RangeEntity.__index = RangeEntity


function RangeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RangeEntity)
  self._name = "range"
  self._client = client
...
function RangeEntity:get_name()
  return self._name
end


function RangeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RangeEntity.new(self._client, opts)
end
...
function RangeEntity:mark_deleted()
  self._deleted = true
end


function RangeEntity:deleted()
  return true == self._deleted
end


function RangeEntity:data_set(args)
  if args ~= nil then
...
function RangeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RangeEntity:match_set(args)
  if args ~= nil then
...
function RangeEntity:match_get()
...
function RangeEntity:stream(action, args, callopts)
...
function RangeEntity:_run_op(ctx, post_done)
  local utility = self._utility
