-- IpinfoDeveloper SDK PrivacyExtended entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PrivacyExtendedEntity = {}
PrivacyExtendedEntity.__index = PrivacyExtendedEntity


function PrivacyExtendedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PrivacyExtendedEntity)
  self._name = "privacy_extended"
  self._client = client
...
function PrivacyExtendedEntity:get_name()
  return self._name
end


function PrivacyExtendedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PrivacyExtendedEntity.new(self._client, opts)
end
...
function PrivacyExtendedEntity:mark_deleted()
  self._deleted = true
end


function PrivacyExtendedEntity:deleted()
  return true == self._deleted
end


function PrivacyExtendedEntity:data_set(args)
  if args ~= nil then
...
function PrivacyExtendedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PrivacyExtendedEntity:match_set(args)
  if args ~= nil then
...
function PrivacyExtendedEntity:match_get()
...
function PrivacyExtendedEntity:stream(action, args, callopts)
...
function PrivacyExtendedEntity:_run_op(ctx, post_done)
  local utility = self._utility
