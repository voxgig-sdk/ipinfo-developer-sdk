...
local MaxEntity = {}
MaxEntity.__index = MaxEntity


function MaxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MaxEntity)
  self._name = "max"
  self._client = client
...
function MaxEntity:get_name()
  return self._name
end


function MaxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MaxEntity.new(self._client, opts)
end
...
function MaxEntity:mark_deleted()
  self._deleted = true
end


function MaxEntity:deleted()
  return true == self._deleted
end


function MaxEntity:data_set(args)
  if args ~= nil then
...
function MaxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MaxEntity:match_set(args)
  if args ~= nil then
...
function MaxEntity:stream(action, args, callopts)
...
function MaxEntity:_run_op(ctx, post_done)
  local utility = self._utility
