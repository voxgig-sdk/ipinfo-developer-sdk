...
local WhoisIpEntity = {}
WhoisIpEntity.__index = WhoisIpEntity


function WhoisIpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisIpEntity)
  self._name = "whois_ip"
  self._client = client
...
function WhoisIpEntity:get_name()
  return self._name
end


function WhoisIpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisIpEntity.new(self._client, opts)
end
...
function WhoisIpEntity:mark_deleted()
  self._deleted = true
end


function WhoisIpEntity:deleted()
  return true == self._deleted
end


function WhoisIpEntity:data_set(args)
  if args ~= nil then
...
function WhoisIpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisIpEntity:match_set(args)
  if args ~= nil then
...
function WhoisIpEntity:match_get()
...
function WhoisIpEntity:stream(action, args, callopts)
...
---@param reqmatch WhoisIpLoadMatch
---@param ctrl? table
---@return WhoisIp
---@return string? err
function WhoisIpEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WhoisIpEntity:_run_op(ctx, post_done)
  local utility = self._utility
