...
local CarrierEntity = {}
CarrierEntity.__index = CarrierEntity


function CarrierEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CarrierEntity)
  self._name = "carrier"
  self._client = client
...
function CarrierEntity:get_name()
  return self._name
end


function CarrierEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CarrierEntity.new(self._client, opts)
end
...
function CarrierEntity:mark_deleted()
  self._deleted = true
end


function CarrierEntity:deleted()
  return true == self._deleted
end


function CarrierEntity:data_set(args)
  if args ~= nil then
...
function CarrierEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CarrierEntity:match_set(args)
  if args ~= nil then
...
function CarrierEntity:match_get()
...
function CarrierEntity:stream(action, args, callopts)
...
---@param reqmatch CarrierLoadMatch
---@param ctrl? table
---@return Carrier
---@return string? err
function CarrierEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CarrierEntity:_run_op(ctx, post_done)
  local utility = self._utility
