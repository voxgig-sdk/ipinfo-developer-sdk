...
local ResidentialProxyEntity = {}
ResidentialProxyEntity.__index = ResidentialProxyEntity


function ResidentialProxyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResidentialProxyEntity)
  self._name = "residential_proxy"
  self._client = client
...
function ResidentialProxyEntity:get_name()
  return self._name
end


function ResidentialProxyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResidentialProxyEntity.new(self._client, opts)
end
...
function ResidentialProxyEntity:mark_deleted()
  self._deleted = true
end


function ResidentialProxyEntity:deleted()
  return true == self._deleted
end


function ResidentialProxyEntity:data_set(args)
  if args ~= nil then
...
function ResidentialProxyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResidentialProxyEntity:match_set(args)
  if args ~= nil then
...
function ResidentialProxyEntity:match_get()
...
function ResidentialProxyEntity:stream(action, args, callopts)
...
---@param reqmatch ResidentialProxyLoadMatch
---@param ctrl? table
---@return ResidentialProxy
---@return string? err
function ResidentialProxyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ResidentialProxyEntity:_run_op(ctx, post_done)
  local utility = self._utility
