...
local WhoisPocEntity = {}
WhoisPocEntity.__index = WhoisPocEntity


function WhoisPocEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisPocEntity)
  self._name = "whois_poc"
  self._client = client
...
function WhoisPocEntity:get_name()
  return self._name
end


function WhoisPocEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisPocEntity.new(self._client, opts)
end
...
function WhoisPocEntity:mark_deleted()
  self._deleted = true
end


function WhoisPocEntity:deleted()
  return true == self._deleted
end


function WhoisPocEntity:data_set(args)
  if args ~= nil then
...
function WhoisPocEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisPocEntity:match_set(args)
  if args ~= nil then
...
function WhoisPocEntity:match_get()
...
function WhoisPocEntity:stream(action, args, callopts)
...
---@param reqmatch WhoisPocLoadMatch
---@param ctrl? table
---@return WhoisPoc
---@return string? err
function WhoisPocEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WhoisPocEntity:_run_op(ctx, post_done)
  local utility = self._utility
