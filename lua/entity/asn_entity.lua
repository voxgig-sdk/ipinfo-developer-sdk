...
local AsnEntity = {}
AsnEntity.__index = AsnEntity


function AsnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AsnEntity)
  self._name = "asn"
  self._client = client
...
function AsnEntity:get_name()
  return self._name
end


function AsnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AsnEntity.new(self._client, opts)
end
...
function AsnEntity:mark_deleted()
  self._deleted = true
end


function AsnEntity:deleted()
  return true == self._deleted
end


function AsnEntity:data_set(args)
  if args ~= nil then
...
function AsnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AsnEntity:match_set(args)
  if args ~= nil then
...
function AsnEntity:stream(action, args, callopts)
...
function AsnEntity:_run_op(ctx, post_done)
  local utility = self._utility
