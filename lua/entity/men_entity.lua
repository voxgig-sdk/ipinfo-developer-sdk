...
local MenEntity = {}
MenEntity.__index = MenEntity


function MenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MenEntity)
  self._name = "men"
  self._client = client
...
function MenEntity:get_name()
  return self._name
end


function MenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MenEntity.new(self._client, opts)
end
...
function MenEntity:mark_deleted()
  self._deleted = true
end


function MenEntity:deleted()
  return true == self._deleted
end


function MenEntity:data_set(args)
  if args ~= nil then
...
function MenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MenEntity:match_set(args)
  if args ~= nil then
...
function MenEntity:stream(action, args, callopts)
...
---@param reqmatch MenLoadMatch
---@param ctrl? table
---@return Men
---@return string? err
function MenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MenEntity:_run_op(ctx, post_done)
  local utility = self._utility
