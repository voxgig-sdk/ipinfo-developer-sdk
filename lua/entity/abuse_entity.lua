...
local AbuseEntity = {}
AbuseEntity.__index = AbuseEntity


function AbuseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AbuseEntity)
  self._name = "abuse"
  self._client = client
...
function AbuseEntity:get_name()
  return self._name
end


function AbuseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AbuseEntity.new(self._client, opts)
end
...
function AbuseEntity:mark_deleted()
  self._deleted = true
end


function AbuseEntity:deleted()
  return true == self._deleted
end


function AbuseEntity:data_set(args)
  if args ~= nil then
...
function AbuseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AbuseEntity:match_set(args)
  if args ~= nil then
...
function AbuseEntity:match_get()
...
function AbuseEntity:stream(action, args, callopts)
...
function AbuseEntity:_run_op(ctx, post_done)
  local utility = self._utility
