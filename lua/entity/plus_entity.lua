...
local PlusEntity = {}
PlusEntity.__index = PlusEntity


function PlusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlusEntity)
  self._name = "plus"
  self._client = client
...
function PlusEntity:get_name()
  return self._name
end


function PlusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlusEntity.new(self._client, opts)
end
...
function PlusEntity:mark_deleted()
  self._deleted = true
end


function PlusEntity:deleted()
  return true == self._deleted
end


function PlusEntity:data_set(args)
  if args ~= nil then
...
function PlusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlusEntity:match_set(args)
  if args ~= nil then
...
function PlusEntity:stream(action, args, callopts)
...
---@param reqmatch PlusLoadMatch
---@param ctrl? table
---@return Plus
---@return string? err
function PlusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PlusEntity:_run_op(ctx, post_done)
  local utility = self._utility
