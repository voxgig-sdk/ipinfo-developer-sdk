...
local WhoisOrgEntity = {}
WhoisOrgEntity.__index = WhoisOrgEntity


function WhoisOrgEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisOrgEntity)
  self._name = "whois_org"
  self._client = client
...
function WhoisOrgEntity:get_name()
  return self._name
end


function WhoisOrgEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisOrgEntity.new(self._client, opts)
end
...
function WhoisOrgEntity:mark_deleted()
  self._deleted = true
end


function WhoisOrgEntity:deleted()
  return true == self._deleted
end


function WhoisOrgEntity:data_set(args)
  if args ~= nil then
...
function WhoisOrgEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisOrgEntity:match_set(args)
  if args ~= nil then
...
function WhoisOrgEntity:match_get()
...
function WhoisOrgEntity:stream(action, args, callopts)
...
---@param reqmatch WhoisOrgLoadMatch
---@param ctrl? table
---@return WhoisOrg
---@return string? err
function WhoisOrgEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WhoisOrgEntity:_run_op(ctx, post_done)
  local utility = self._utility
