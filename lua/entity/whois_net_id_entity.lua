...
local WhoisNetIdEntity = {}
WhoisNetIdEntity.__index = WhoisNetIdEntity


function WhoisNetIdEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisNetIdEntity)
  self._name = "whois_net_id"
  self._client = client
...
function WhoisNetIdEntity:get_name()
  return self._name
end


function WhoisNetIdEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisNetIdEntity.new(self._client, opts)
end
...
function WhoisNetIdEntity:mark_deleted()
  self._deleted = true
end


function WhoisNetIdEntity:deleted()
  return true == self._deleted
end


function WhoisNetIdEntity:data_set(args)
  if args ~= nil then
...
function WhoisNetIdEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisNetIdEntity:match_set(args)
  if args ~= nil then
...
function WhoisNetIdEntity:match_get()
...
function WhoisNetIdEntity:stream(action, args, callopts)
...
function WhoisNetIdEntity:_run_op(ctx, post_done)
  local utility = self._utility
