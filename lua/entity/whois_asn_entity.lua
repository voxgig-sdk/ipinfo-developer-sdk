...
local WhoisAsnEntity = {}
WhoisAsnEntity.__index = WhoisAsnEntity


function WhoisAsnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoisAsnEntity)
  self._name = "whois_asn"
  self._client = client
...
function WhoisAsnEntity:get_name()
  return self._name
end


function WhoisAsnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoisAsnEntity.new(self._client, opts)
end
...
function WhoisAsnEntity:mark_deleted()
  self._deleted = true
end


function WhoisAsnEntity:deleted()
  return true == self._deleted
end


function WhoisAsnEntity:data_set(args)
  if args ~= nil then
...
function WhoisAsnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoisAsnEntity:match_set(args)
  if args ~= nil then
...
function WhoisAsnEntity:match_get()
...
function WhoisAsnEntity:stream(action, args, callopts)
...
function WhoisAsnEntity:_run_op(ctx, post_done)
  local utility = self._utility
