...
local IpinfoPlusEntity = {}
IpinfoPlusEntity.__index = IpinfoPlusEntity


function IpinfoPlusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpinfoPlusEntity)
  self._name = "ipinfo_plus"
  self._client = client
...
function IpinfoPlusEntity:get_name()
  return self._name
end


function IpinfoPlusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpinfoPlusEntity.new(self._client, opts)
end
...
function IpinfoPlusEntity:mark_deleted()
  self._deleted = true
end


function IpinfoPlusEntity:deleted()
  return true == self._deleted
end


function IpinfoPlusEntity:data_set(args)
  if args ~= nil then
...
function IpinfoPlusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpinfoPlusEntity:match_set(args)
  if args ~= nil then
...
function IpinfoPlusEntity:match_get()
...
function IpinfoPlusEntity:stream(action, args, callopts)
...
function IpinfoPlusEntity:_run_op(ctx, post_done)
  local utility = self._utility
