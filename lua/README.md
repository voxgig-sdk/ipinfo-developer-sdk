...
The Lua SDK for the IpinfoDeveloper API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Abuse(nil):load({ id = "example_id" }, nil)
...
local result, err = client:IpinfoDeveloper(nil):load(
  { id = "test01" }, nil
)
