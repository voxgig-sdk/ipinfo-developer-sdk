...
require("ipinfo-developer_types")
...
function IpinfoDeveloperSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function IpinfoDeveloperSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "IpinfoDeveloperSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function IpinfoDeveloperSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function IpinfoDeveloperSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "IpinfoDeveloperSDK: graphql: " .. msg
    res.graphql = errors
  end
