...
describe('PrivacyExtendedEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'privacy_extended.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_PRIVACY_EXTENDED_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['privacy_extended01','privacy_extended02','privacy_extended03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['IPINFO_DEVELOPER_TEST_PRIVACY_EXTENDED_ENTID']
