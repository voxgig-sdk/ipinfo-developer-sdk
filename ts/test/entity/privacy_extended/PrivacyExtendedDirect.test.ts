...
describe('PrivacyExtendedDirect', async () => {
  ...
  test('direct-list-privacy_extended', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-privacy_extended', setup.live)) return
    if (skipIfMissingIds(t, setup, ["ip01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IPINFO_DEVELOPER_TEST_PRIVACY_EXTENDED_ENTID']
