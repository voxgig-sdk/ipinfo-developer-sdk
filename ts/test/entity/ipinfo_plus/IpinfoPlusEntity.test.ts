...
describe('IpinfoPlusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'ipinfo_plus.' + op, live)) return
    }
    ...
    let ipinfo_plus_ref01_data = Object.values(setup.data.existing.ipinfo_plus)[0] as any

    // LOAD
    const ipinfo_plus_ref01_ent = client.IpinfoPlus()
    const ipinfo_plus_ref01_match_dt0: any = {}
    ...
    assert(ipinfo_plus_ref01_data_dt0.id === ipinfo_plus_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/ipinfo_plus/IpinfoPlusTestData.json')
  ...
  const idmapEnvVal = process.env['IPINFO_DEVELOPER_TEST_IPINFO_PLUS_ENTID']
