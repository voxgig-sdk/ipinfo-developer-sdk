...
describe('IpinfoPlusDirect', async () => {
  ...
  test('direct-load-ipinfo_plus', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ipinfo_plus', setup.live)) return
    if (skipIfMissingIds(t, setup, ["field01","ip01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IPINFO_DEVELOPER_TEST_IPINFO_PLUS_ENTID']
