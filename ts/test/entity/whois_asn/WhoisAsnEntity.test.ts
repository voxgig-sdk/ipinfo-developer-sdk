...
describe('WhoisAsnEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'whois_asn.' + op, live)) return
    }
    ...
    const whois_asn_ref01_list = await whois_asn_ref01_ent.list(whois_asn_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['whois_asn01','whois_asn02','whois_asn03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['IPINFO_DEVELOPER_TEST_WHOIS_ASN_ENTID']
