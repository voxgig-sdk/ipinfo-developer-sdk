...
describe('WhoisAsnDirect', async () => {
  ...
  test('direct-list-whois_asn', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-whois_asn', setup.live)) return
    if (skipIfMissingIds(t, setup, ["asn01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IPINFO_DEVELOPER_TEST_WHOIS_ASN_ENTID']
