...
describe('PrivacyDirect', async () => {
  ...
  test('direct-load-privacy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-privacy', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IPINFO_DEVELOPER_TEST_PRIVACY_ENTID']
