...
describe('WhoisIpDirect', async () => {
  ...
  test('direct-load-whois_ip', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-whois_ip', setup.live)) return
    if (skipIfMissingIds(t, setup, ["whoisip01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IPINFO_DEVELOPER_TEST_WHOIS_IP_ENTID']
