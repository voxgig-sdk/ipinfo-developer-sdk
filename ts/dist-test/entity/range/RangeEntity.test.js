...
(0, node_test_1.describe)('RangeEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'range.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set IPINFO_DEVELOPER_TEST_RANGE_ENTID JSON to run live');
            return;
        }
        ...
        let range_ref01_data = Object.values(setup.data.existing.range)[0];
        // LOAD
        const range_ref01_ent = client.Range();
        const range_ref01_match_dt0 = {};
        ...
        (0, node_assert_1.default)(range_ref01_data_dt0.id === range_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/range/RangeTestData.json');
    ...
    let idmap = transform(['range01', 'range02', 'range03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['IPINFO_DEVELOPER_TEST_RANGE_ENTID'];
...
//# sourceMappingURL=RangeEntity.test.js.map