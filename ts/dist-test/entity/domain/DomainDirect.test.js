...
(0, node_test_1.describe)('DomainDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-domain', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-domain', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['IPINFO_DEVELOPER_TEST_DOMAIN_ENTID'];
