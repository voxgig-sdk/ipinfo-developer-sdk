...
// IpinfoDeveloper SDK retry feature
...
class IpinfoDeveloperRetryFeature extends IpinfoDeveloperBaseFeature
{
    ...
    public function init(IpinfoDeveloperContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpinfoDeveloperContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpinfoDeveloperContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpinfoDeveloperContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
