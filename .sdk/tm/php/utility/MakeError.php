...
class IpinfoDeveloperMakeError
{
    public static function call(?IpinfoDeveloperContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpinfoDeveloperContext([], null);
        }
        $op = $ctx->op ?? new IpinfoDeveloperOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpinfoDeveloperResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpinfoDeveloperError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpinfoDeveloperError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IpinfoDeveloperError) {
